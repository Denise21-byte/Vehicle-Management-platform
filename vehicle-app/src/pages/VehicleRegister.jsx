...
import { Car, User, FileText, CheckCircle, ArrowRight, ArrowLeft } from 'lucide-react';
...
export default function VehicleRegister() {
  ...
  const mutation = useMutation({
    ...
    onError: (err) => {
      ...
      if (Array.isArray(errs)) errs.forEach(e => toast.error(e.message || e));
  ...
  const onStepSubmit = (data) => {
    ...
    if (step < STEPS.length - 1) { setFormData(updated); setStep(s => s + 1); }
    else mutation.mutate(updated);
  };
    ...
    <div style={{ padding: '40px 20px', maxWidth: '780px', margin: '0 auto' }}>
          ...
          return (
            <div key={s.label} style={{ flex: 1 }}>
              <div style={{
                ...
                <div>
                  <p style={{ fontSize: '11px', color: 'var(--text-muted)', fontFamily: 'Syne' }}>Step {i+1}</p>
                  <p style={{ fontSize: '13px', fontWeight: 600, fontFamily: 'Syne', color: active ? 'var(--accent)' : done ? '#10B981' : 'var(--text-secondary)' }}>
                    {s.label}
                  </p>
                </div>
              </div>
      ...
      <div className="card fade-up" style={{ padding: '32px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '28px', paddingBottom: '20px', borderBottom: '1px solid var(--border)' }}>
          ...
          <div>
            <h2 style={{ fontFamily: 'Syne', fontSize: '20px', fontWeight: 700 }}>{STEPS[step].label}</h2>
            <p style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Step {step+1} of {STEPS.length}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onStepSubmit)}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '18px' }}>
            ...
            {step === 0 && <>
              ...
              <InputField label="Engine Capacity (cc)" registration={register('engineCapacity')} error={errors.engineCapacity} type="number" placeholder="e.g. 1500" />
              ...
              <SelectField label="Vehicle Type"     registration={register('vehicleType')}      error={errors.vehicleType}      options={['ELECTRIC','SUV','TRUCK','MOTORCYCLE','BUS','VAN','PICKUP','OTHER']} />
              <SelectField label="Fuel Type"        registration={register('fuelType')}         error={errors.fuelType}         options={['PETROL','DIESEL','ELECTRIC','HYBRID','GAS','OTHER']} />
              <SelectField label="Purpose"          registration={register('purpose')}          error={errors.purpose}          options={['PERSONAL','COMMERCIAL','TAXI','GOVERNMENT']} />
              <SelectField label="Vehicle Status"   registration={register('status')}           error={errors.status}           options={['NEW','USED','REBUILT']} />
            </>}

            {/* STEP 1 */}
            {step === 1 && <>
              <InputField label="Owner Full Name"   registration={register('ownerName')}       error={errors.ownerName}       placeholder="e.g. John Doe" />
              <SelectField label="Owner Type"       registration={register('ownerType')}       error={errors.ownerType}       options={['INDIVIDUAL','COMPANY','NGO','GOVERNMENT']} />
              <div style={{ gridColumn: '1/-1' }}>
                <InputField label="Address"         registration={register('address')}         error={errors.address}         placeholder="Full address" />
              </div>
              <InputField label="National ID"       registration={register('nationalId')}      error={errors.nationalId}      placeholder="16 digits" hint="Exactly 16 digits required" />
              <InputField label="Mobile Number"     registration={register('mobile')}          error={errors.mobile}          placeholder="10 digits" hint="Exactly 10 digits required" />
            ...
            {step === 2 && <>
              ...
              <SelectField label="Plate Type"         registration={register('plateType')}           error={errors.plateType}           options={['PRIVATE','COMMERCIAL','GOVERNMENT','DIPLOMATIC','PERSONALIZED']} />
              ...
              <SelectField label="Registration Status" registration={register('registrationStatus')} error={errors.registrationStatus}  options={['ACTIVE','SUSPENDED','EXPIRED','PENDING']} />
              <InputField label="Customs Reference"   registration={register('customsRef')}          error={errors.customsRef}          placeholder="Customs ref number" />
              <InputField label="Proof of Ownership"  registration={register('proofOfOwnership')}    error={errors.proofOfOwnership}    placeholder="Document reference" />
              <InputField label="Roadworthy Cert"     registration={register('roadworthyCert')}      error={errors.roadworthyCert}      placeholder="Certificate number" />
              <InputField label="Policy Number"       registration={register('policyNumber')}        error={errors.policyNumber}        placeholder="Insurance policy no." />
              ...
              <InputField label="Insurance Expiry"    registration={register('insuranceExpiryDate')} error={errors.insuranceExpiryDate} type="date" />
              <SelectField label="Insurance Status"   registration={register('insuranceStatus')}     error={errors.insuranceStatus}     options={['ACTIVE','SUSPENDED','EXPIRED']} />
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '28px', paddingTop: '20px', borderTop: '1px solid var(--border)' }}>
            {step > 0 ? (
              <button type="button" onClick={() => setStep(s => s - 1)} className="btn-ghost" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <ArrowLeft size={14} /> Back
              </button>
            ) : <div />}

            <button type="submit" disabled={mutation.isPending} className="btn-primary" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              {step < STEPS.length - 1
